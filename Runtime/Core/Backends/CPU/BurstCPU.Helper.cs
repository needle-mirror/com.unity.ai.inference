...
namespace Unity.InferenceEngine
{
    static class BurstSchedulingHelper
    {
        ...
        static unsafe JobHandle ScheduleBatchOInternal<T>(ref this T jobData,
            JobHandle fenceBeforeJobStart,
            void* ptrO,
            int arrayLength, int innerloopBatchCount)
            where T : struct, IParallelForBatch, CPUBackend.IJobResourceDeclarationO
        {
            jobData.O = new CPUBackend.ReadWriteMemResource() { ptr = ptrO };
            return jobData.ScheduleBatch(arrayLength, innerloopBatchCount, fenceBeforeJobStart);
        }
        ...
        static JobHandle GetFenceBeforeJobStartO(
            IDependableMemoryResource pinO)
        {
            return pinO.reuse;
        }
        ...
        static void SetOFences(this JobHandle jobFence,
            IDependableMemoryResource pinO)
        {
            pinO.fence = jobFence;
        }
        ...
        public static unsafe JobHandle ScheduleBatchO<T>(ref this T jobData,
            IDependableMemoryResource O,
            int arrayLength, int innerloopBatchCount)
            where T : struct, IParallelForBatch, CPUBackend.IJobResourceDeclarationO
        {
            var fenceBeforeJobStart = GetFenceBeforeJobStartO(O);
            var jobFence = jobData.ScheduleBatchOInternal(fenceBeforeJobStart, O.rawPtr, arrayLength, innerloopBatchCount);
            jobFence.SetOFences(O);
            return jobFence;
        }
