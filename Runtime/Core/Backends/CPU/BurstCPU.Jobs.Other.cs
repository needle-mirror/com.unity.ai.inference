...
namespace Unity.InferenceEngine
{
    partial class CPUBackend
    {
        ...
        unsafe struct OneHotJob : IJobParallelFor, IJobResourceDeclarationXO
        {
            ...
            public int stridesToAxis;
            public int axisDim;
            public int negativeIndexOffset;

            public void Execute(int threadIdx)
            {
                int i = threadIdx;
                int innerIdx = i % stridesToAxis;
                i /= stridesToAxis;
                int axisIdx = i % axisDim;
                i /= axisDim;
                int outerIdx = i;
        ...
        unsafe struct ScatterNDFloatJob : IJobParallelFor, IJobResourceDeclarationXSBO
        {
            ...
            public void Execute(int threadIdx)
            {
                ...
                if (reduction == Layers.ScatterReductionMode.None)
                    ...
                    Optr[indexO * updatesLength + k] *= vw;
                else if (reduction == Layers.ScatterReductionMode.Max)
                    Optr[indexO * updatesLength + k] = math.max(vw, Optr[indexO * updatesLength + k]);
                else if (reduction == Layers.ScatterReductionMode.Min)
                    Optr[indexO * updatesLength + k] = math.min(vw, Optr[indexO * updatesLength + k]);
            }
        }
        ...
        [BurstCompile(OptimizeFor = OptimizeFor.Performance, FloatMode = FloatMode.Fast, FloatPrecision = FloatPrecision.Standard, CompileSynchronously = true)]
        unsafe struct TopKFloatJob : IJobParallelFor
        {
        ...
        [BurstCompile(OptimizeFor = OptimizeFor.Performance, FloatMode = FloatMode.Fast, FloatPrecision = FloatPrecision.Standard, CompileSynchronously = true)]
        unsafe struct TopKIntJob : IJobParallelFor
        {
            [NoAlias][NativeDisableUnsafePtrRestriction][Collections.ReadOnly] public int* Xptr;
            [NoAlias][NativeDisableUnsafePtrRestriction] public int* Valuesptr;
            [NoAlias][NativeDisableUnsafePtrRestriction] public int* Indicesptr;
            ...
            public void Execute(int index)
            {
                ...
                int* Xp = Xptr + outerIndex * reduceLength * innerLength + innerIndex;
                int* Vp = Valuesptr + outerIndex * maxK * innerLength + innerIndex;
                int* Ip = Indicesptr + outerIndex * maxK * innerLength + innerIndex;
