...
namespace Unity.InferenceEngine
{
    ...
    partial class GPUComputeBackend : IBackend
    {
        ...
        public virtual void Conv(Tensor<float> X, Tensor<float> K, Tensor<float> B, Tensor<float> O, int groups, Span<int> strides, Span<int> pads, Span<int> dilations, Layers.FusableActivation fusedActivation)
        {
            ...
            if (!ComputeInfo.IsDirect3DAPI || (X.shape.rank == 5))
            {
                ConvGeneric(X, K, B, O, strides, pads, dilations, fusedActivation);
                return;
            }
        ...
        void ConvGeneric(Tensor<float> X, Tensor<float> K, Tensor<float> B, Tensor<float> O, Span<int> strides, Span<int> pads, Span<int> dilations, Layers.FusableActivation fusedActivation)
        {
            bool unitStrides;
        ...
        void ConvTransposeGeneric(Tensor<float> X, Tensor<float> W, Tensor<float> B, Tensor<float> O, Span<int> strides, Span<int> pads, Span<int> outputAdjustment, Layers.FusableActivation fusedActivation)
        {
            ...
            Span<int> paddings = stackalloc int[4] { 0, 0, 0, 0 };
            ...
            cb.SetInt4(fn, k_ID__Pad, paddings);
            cb.SetInt4(fn, k_ID__Stride, strides);
            cb.SetInt4(fn, k_ID__Dilation, dilations);
        ...
        {
            var fn = ComputeFunctions.k_Mish;
            cb.SetTensorAsBuffer(fn, k_ID_X_float_ptr, Pin(X));
            cb.SetTensorAsBuffer(fn, k_ID_O_float_ptr, Pin(O));
            cb.UnrolledDispatchFast(fn, O.shape.length);
        }
        ...
        void ScatterNDGeneric(Tensor X, Tensor<int> indices, Tensor updates, Tensor O, Layers.ScatterReductionMode reduction)
        {
            MemCopy(X, O);
            ...
            var fn = ComputeFunctions.k_ScatterND;
            cb.SetKeyword(fn.shader, new LocalKeyword(fn.shader, "INT"), X.dataType == DataType.Int);
            cb.SetKeyword(fn.shader, new LocalKeyword(fn.shader, "ReduceAdd"), reduction == Layers.ScatterReductionMode.Add);
            cb.SetKeyword(fn.shader, new LocalKeyword(fn.shader, "ReduceMul"), reduction == Layers.ScatterReductionMode.Mul);
            cb.SetKeyword(fn.shader, new LocalKeyword(fn.shader, "ReduceMax"), reduction == Layers.ScatterReductionMode.Max);
            cb.SetKeyword(fn.shader, new LocalKeyword(fn.shader, "ReduceMin"), reduction == Layers.ScatterReductionMode.Min);
        ...
        {
            ScatterNDGeneric(X, indices, updates, O, reduction);
        }
        ...
        public void OneHot(Tensor<int> X, Tensor<int> O, int axis, int depth, int offValue, int onValue, bool allowNegativeIndexes)
        {
            ...
            cb.SetComputeIntParam(fn.shader, k_ID_negativeIndexOffset, allowNegativeIndexes ? depth : 0);
        ...
        /// <inheritdoc/>
        void TopKGeneric(Tensor X, Tensor values, Tensor<int> indices, int k, int axis, bool largest)
        {
            int reduceLength = X.shape[axis];
            int innerLength = X.shape.Strides(axis);
        ...
        {
            TopKGeneric(X, values, indices, k, axis, largest);
        }
        ...
        {
            var fn = ComputeFunctions.k_RoiAlign;
            cb.SetKeyword(fn.shader, new LocalKeyword(fn.shader, "AVG"), mode == Layers.RoiPoolingMode.Avg);
            cb.SetKeyword(fn.shader, new LocalKeyword(fn.shader, "HALFPIXEL"), coordinateTransformationMode == Layers.RoiCoordinateTransformationMode.HalfPixel);
